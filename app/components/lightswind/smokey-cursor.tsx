...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    // Coordinates relative to the canvas box (so the effect is scoped to its
    // container, not the whole viewport).
    function pointerX(clientX: number) {
      const r = canvas!.getBoundingClientRect();
      return scaleByPixelRatio(clientX - r.left);
    }
    function pointerY(clientY: number) {
      const r = canvas!.getBoundingClientRect();
      return scaleByPixelRatio(clientY - r.top);
    }
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      const posX = pointerX(e.clientX);
      const posY = pointerY(e.clientY);
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const posX = pointerX(touches[i].clientX);
        const posY = pointerY(touches[i].clientY);
        updateFrame();
